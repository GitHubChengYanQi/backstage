package cn.atsoft.dasheng.storehousePosition.service.impl;
...
import cn.atsoft.dasheng.storehousePosition.mapper.RestStorehousePositionsMapper;
 ...
 */
@Service
public class RestStorehousePositionsServiceImpl extends ServiceImpl<RestStorehousePositionsMapper, RestStorehousePositions> implements RestStorehousePositionsService {
    @Autowired
...
//    private ProductionPickListsCartService cartService;
    ...
    @Override
    public void add(RestStorehousePositionsParam param) {
...
//        }
        ...
        RestStorehousePositions entity = getEntity(param);
        this.save(entity);
    ...
    public List<Long> getChildrens(Long id) {
        ...
        List<RestStorehousePositions> positions = this.query().like("pid", id).eq("display", 1).list();

        List<Long> childrenList = new ArrayList<>();
        for (RestStorehousePositions storehousePositions : positions) {
...
//            }
    ...
    public void updateChildren(Long id) {
        RestStorehousePositions position = this.getById(id);
        if(ToolUtil.isEmpty(position)) return;

        List<RestStorehousePositions> positions = this.query().like("pid", id).eq("display", 1).list();

        List<Long> children = new ArrayList<>();
        children.addAll(positions.stream().map(RestStorehousePositions::getStorehousePositionsId).collect(Collectors.toList()));
    ...
    public Long judgePosition(Long positionId) {
        ...
        RestStorehousePositions positions = this.query().eq("pid", positionId).one();
    ...
    @Override
    public List<RestStorehousePositionsResult> details(List<Long> positionIds) {
        if (ToolUtil.isEmpty(positionIds)) {
            return new ArrayList<>();
        }
        List<RestStorehousePositions> positions = this.listByIds(positionIds);
        if (ToolUtil.isEmpty(positions)) {
            return new ArrayList<>();
        }

        List<RestStorehousePositionsResult> positionsResults = BeanUtil.copyToList(positions, RestStorehousePositionsResult.class);
        format(positionsResults);
        return positionsResults;
    }

    @Override
    public void delete(RestStorehousePositionsParam param) {
        List<RestStorehousePositions> list = this.query().eq("pid", param.getStorehousePositionsId()).eq("display", 1).list();
        if (ToolUtil.isNotEmpty(list)) {
            throw new ServiceException(500, "当前仓位不能删除");
        }
        List<RestStockDetails> details = stockDetailsService.query().eq("storehouse_positions_id", param.getStorehousePositionsId()).eq("display", 1).list();
        if (ToolUtil.isNotEmpty(details)) {
            throw new ServiceException(500, "库位已被使用，不可以删除");
        }
        RestStorehousePositions positions = this.getById(param.getStorehousePositionsId());
    ...
    @Override
    @Transactional
    public void update(RestStorehousePositionsParam param) {

//        RestStorehousePositions oldEntity = getOldEntity(param);
//        RestStorehousePositions newEntity = getEntity(param);
...
//        updateChildren(newEntity.getPid());
    ...
    @Override
    public List<RestStorehousePositionsResult> selectBySku(RestStorehousePositionsParam param) {
        return null;
    }
...
//
//        }
//        List<RestStorehousePositions> positions = positionIds.size() == 0 ? new ArrayList<>() : this.listByIds(positionIds);
//        List<RestStorehousePositionsResult> positionsResults = BeanUtil.copyToList(positions, RestStorehousePositionsResult.class, new CopyOptions());
...
//        }
//
//        for (RestStorehousePositionsResult positionsResult : positionsResults) {
...
//    }
     ...
     * @return
     */
    private boolean positionSet(List<RestStorehousePositionsResult> positionsResultList, RestStorehousePositionsResult newPosition) {

        for (RestStorehousePositionsResult positionsResult : positionsResultList) {
...
//    }
    ...
    @Override
    public RestStorehousePositionsResult positionsResultById(Long Id) {

//        RestStorehousePositions storehousePositions = this.getById(Id);
//
//        if (ToolUtil.isNotEmpty(storehousePositions)) {
//            RestStorehousePositionsResult positionsResult = new RestStorehousePositionsResult();
...
//    private void returnPrintTemplate(RestStorehousePositionsResult param) {
...
//    }
    ...
    private String getParent(Long id) {
        RestStorehousePositions positions = this.getById(id);
        RestStorehouse storehouse = storehouseService.getById(positions.getStorehouseId());
        List<RestStorehousePositions> storehousePositionsList = this.query().eq("storehouse_id", positions.getStorehouseId()).eq("display", 1).list();
...
//    }
    ...
    public List<Long> getLoopPositionIds(Long positionId) {
        ...
        List<RestStorehousePositions> positions = this.query().eq("display", 1).list();

        for (RestStorehousePositions position : positions) {
    ...
    @Override
    public Map<Long, List<RestStorehousePositionsResult>> getMap(List<Long> skuIds) {
        return null;
    }
     ...
     * @return
     */
    private List<Long> loopChild(RestStorehousePositions position, List<RestStorehousePositions> positions) {
...
//    public Map<Long, List<RestStorehousePositionsResult>> getMap(List<Long> skuIds) {
//
//        List<RestStorehousePositionsBind> positionsBinds = skuIds.size() == 0 ? new ArrayList<>() : storehousePositionsBindService.query().in("sku_id", skuIds).eq("display", 1).list();
//        List<Long> positionIds = new ArrayList<>();
//
//
//        for (RestStorehousePositionsBind positionsBind : positionsBinds) {
//            positionIds.add(positionsBind.getPositionId());
//        }
//
//        List<RestStorehousePositions> positions = positionIds.size() == 0 ? new ArrayList<>() : this.listByIds(positionIds);
//        List<RestStorehousePositionsResult> positionsResults = BeanUtil.copyToList(positions, StorehousePositionsResult.class, new CopyOptions());
//        format(positionsResults);
//        Map<Long, List<RestStorehousePositionsResult>> map = new HashMap<>();
//
//
//        for (RestStorehousePositionsBind positionsBind : positionsBinds) {
//            List<RestStorehousePositionsResult> results = new ArrayList<>();
//            for (RestStorehousePositionsResult positionsResult : positionsResults) {
//                if (positionsBind.getPositionId().equals(positionsResult.getStorehousePositionsId())) {
//                    results.add(positionsResult);
//                }
//            }
//            List<RestStorehousePositionsResult> list = map.get(positionsBind.getSkuId());
//            if (ToolUtil.isNotEmpty(list)) {
//                results.addAll(list);
//            }
...
//    }
    ...
    @Override
    public RestStorehousePositionsResult findBySpec(RestStorehousePositionsParam param) {
        return null;
    }

    @Override
    public List<RestStorehousePositionsResult> findListBySpec(RestStorehousePositionsParam param, DataScope dataScope) {
        return null;
    }
...
//    public List<RestStorehousePositionsResult> findListBySpec(RestStorehousePositionsParam param, DataScope dataScope) {
//
//        List<RestStorehousePositionsResult> storehousePositionsResults = this.baseMapper.customList(param, dataScope);
//        List<Long> positionIds = new ArrayList<>();
//        for (RestStorehousePositionsResult storehousePositions : storehousePositionsResults) {
...
//    }
    ...
    @Override
    public PageInfo<RestStorehousePositionsResult> findPageBySpec(RestStorehousePositionsParam param) {
        Page<RestStorehousePositionsResult> pageContext = getPageContext();
        IPage<RestStorehousePositionsResult> page = this.baseMapper.customPageList(pageContext, param);
     ...
     */
    @Override
    public RestStorehousePositionsResult getDetail(Long id, List<RestStorehousePositions> positions) {
        List<RestStorehousePositionsResult> results = BeanUtil.copyToList(positions, RestStorehousePositionsResult.class, new CopyOptions());
        for (RestStorehousePositionsResult result : results) {
            if (result.getStorehousePositionsId().equals(id)) {
                getSupper(result, results);
                return result;
            }
        }
        return new RestStorehousePositionsResult();
    }

    @Override
    public List<RestStorehousePositionsResult> getDetails(List<Long> ids) {
        if (ToolUtil.isEmpty(ids)) {
            return new ArrayList<>();
        }
        List<RestStorehousePositions> positions = this.list();
        List<RestStorehousePositionsResult> results = BeanUtil.copyToList(positions, RestStorehousePositionsResult.class, new CopyOptions());
        List<RestStorehousePositionsResult> positionsResults = new ArrayList<>();

        for (Long id : ids) {
            for (RestStorehousePositionsResult result : results) {
                if (ToolUtil.isNotEmpty(id) && id.equals(result.getStorehousePositionsId())) {
                    RestStorehousePositionsResult positionsResult = getSupper(result, results);
    ...
    @Override
    public List<RestStorehousePositionsResult> resultsByIds(List<Long> ids) {
        if (ToolUtil.isEmpty(ids)) {
            return new ArrayList<>();
        }
        List<RestStorehousePositions> positions = this.query().in("storehouse_positions_id", ids).eq("display", 1).list();
        List<RestStorehousePositionsResult> results = BeanUtil.copyToList(positions, RestStorehousePositionsResult.class, new CopyOptions());
    ...
    @Override
    public Map<String, Map<String, Object>> takeStock(RestStorehousePositionsParam param) {
        return null;
    }

    private RestStorehousePositionsResult getSupper(RestStorehousePositionsResult result, List<RestStorehousePositionsResult> data) {

        for (RestStorehousePositionsResult datum : data) {
...
//    }
     ...
     */
    @Override
    public RestStorehousePositionsResult detail(Long id) {
        RestStorehousePositions positions = this.getById(id);
        RestStorehousePositionsResult result = new RestStorehousePositionsResult();
        ...
        if (ToolUtil.isNotEmpty(positions)) {
            ...
            List<RestStorehousePositionsResult> results = positionsResults(longs);
     ...
     */
    @Override
    public List<RestStorehousePositionsResult> positionsResults(List<Long> ids) {
        if (ToolUtil.isEmpty(ids)) {
            return new ArrayList<>();
        }
        List<RestStorehousePositions> storehousePositions = this.listByIds(ids);

        return BeanUtil.copyToList(storehousePositions, RestStorehousePositionsResult.class, new CopyOptions());
    }
     ...
     */
    @Override
    public List<RestStorehousePositionsResult> getSupperBySkuId(Long skuId) {
        List<RestStorehousePositionsResult> results = new ArrayList<>();

        QueryWrapper<RestStockDetails> stockDetailsQW = new QueryWrapper<>();
        ...
        List<RestStockDetails> stockDetails = stockDetailsService.list(stockDetailsQW);
        ...
        List<RestStorehousePositions> positionsList = this.list();
        List<RestStorehousePositionsResult> positionsResults = BeanUtil.copyToList(positionsList, RestStorehousePositionsResult.class, new CopyOptions());
     ...
     * 递归取上级
     */
    private void loopSuppler(RestStorehousePositionsResult positionsResult, List<RestStorehousePositionsResult> positionsResults) {
        for (RestStorehousePositionsResult result : positionsResults) {
    ...
    public List<Long> getEndChild(Long positionId) {
        RestStorehousePositions self = this.getById(positionId);
        List<RestStorehousePositions> positions = this.query().eq("storehouse_id", self.getStorehouseId()).eq("display", 1).list();
        List<Long> positionIds = new ArrayList<>();
     ...
     *
     */
    public List<Long> getEndChild(Long positionId, List<RestStorehousePositions> positions) {
        List<Long> positionIds = new ArrayList<>();
        positionIds.add(positionId);
        for (RestStorehousePositions position : positions) {
            if (position.getStorehousePositionsId().equals(positionId)) {
    ...
    private List<Long> loop(Long positionId, List<RestStorehousePositions> positions) {
        List<Long> positionIds = new ArrayList<>();
        for (RestStorehousePositions position : positions) {
     ...
     * @param results
     */
    private void recursive(RestStorehousePositionsResult result, List<RestStorehousePositionsResult> results) {
        List<RestStorehousePositionsResult> positionsList = new ArrayList<>();


        for (RestStorehousePositionsResult storehousePositionsResult : results) {
    ...
    private Serializable getKey(RestStorehousePositionsParam param) {
        return param.getStorehousePositionsId();
    }

    private Page<RestStorehousePositionsResult> getPageContext() {
        return PageFactory.defaultPage();
    }

    private RestStorehousePositions getOldEntity(RestStorehousePositionsParam param) {
        return this.getById(getKey(param));
    }

    private RestStorehousePositions getEntity(RestStorehousePositionsParam param) {
        RestStorehousePositions entity = new RestStorehousePositions();
    ...
    public Map<Long, Long> getHouseByPositionId(List<Long> postitionIds) {
        ...
        List<RestStorehousePositions> positions = postitionIds.size() == 0 ? new ArrayList<>() : this.listByIds(postitionIds);

        for (RestStorehousePositions position : positions) {
            houseIds.add(position.getStorehouseId());
        }
        List<RestStorehouse> storehouses = houseIds.size() == 0 ? new ArrayList<>() : storehouseService.listByIds(houseIds);

        for (RestStorehousePositions position : positions) {
            for (RestStorehouse storehouse : storehouses) {
    ...
    @Override
    public void format(List<RestStorehousePositionsResult> data) {
        ...
        List<RestStorehouse> storehouses = storeIds.size() == 0 ? new ArrayList<>() : storehouseService.query().in("storehouse_id", storeIds).list();
        for (RestStorehousePositionsResult datum : data) {
            for (RestStorehouse storehouse : storehouses) {

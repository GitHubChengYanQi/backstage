...
public class RepairServiceImpl extends ServiceImpl<RepairMapper, Repair> implements RepairService {
    ...
    public void update(RepairParam param) {
        ...
        if (newEntity.getProgress()==0) {
            repairDynamicParam.setContent("待派工....");
            repairDynamicParam.setRepairId(newEntity.getRepairId());
            repairDynamicService.add(repairDynamicParam);
        }else if (newEntity.getProgress()==1){
            repairDynamicParam.setContent("询价中....");
            repairDynamicParam.setRepairId(newEntity.getRepairId());
            repairDynamicService.add(repairDynamicParam);
        }else if (newEntity.getProgress()==2){
            repairDynamicParam.setContent("待支付....");
            repairDynamicParam.setRepairId(newEntity.getRepairId());
            repairDynamicService.add(repairDynamicParam);
        }else if (newEntity.getProgress()==3){
            repairDynamicParam.setContent("实施中....");
            repairDynamicParam.setRepairId(newEntity.getRepairId());
            repairDynamicService.add(repairDynamicParam);
        }else if (newEntity.getProgress()==4){
            repairDynamicParam.setContent("待回访....");
            repairDynamicParam.setRepairId(newEntity.getRepairId());
            repairDynamicService.add(repairDynamicParam);
        }else if (newEntity.getProgress()==5){

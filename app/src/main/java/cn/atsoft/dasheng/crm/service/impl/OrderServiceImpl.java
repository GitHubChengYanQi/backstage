...
//        }
    ...
    private void format(List<OrderResult> data) {
        ...
        List<OrderDetailResult> detailResults = BeanUtil.copyToList(orderIds.size() == 0 ? new ArrayList<>() : detailService.query().in("order_id", orderIds).eq("display", 1).list(), OrderDetailResult.class, new CopyOptions());
